...
DEVICE_PROP Device_Property = { Virtual_Com_Port_init, Virtual_Com_Port_Reset,
		Virtual_Com_Port_Status_In, Virtual_Com_Port_Status_Out,
		Virtual_Com_Port_Data_Setup, Virtual_Com_Port_NoData_Setup,
		Virtual_Com_Port_Get_Interface_Setting,
		Virtual_Com_Port_GetDeviceDescriptor,
		Virtual_Com_Port_GetConfigDescriptor,
		Virtual_Com_Port_GetStringDescriptor, 0, 0x40 /*MAX PACKET SIZE*/
};
...
ONE_DESCRIPTOR String_Descriptor[4] = { {
		(uint8_t*) Virtual_Com_Port_StringLangID,
...
void sendEventConnected(uint16_t bitmap) {
	connectionCount += bitmap;
}
#include "Usb.h"
RESULT Virtual_Com_Port_NoData_Setup(uint8_t RequestNo) {
	...
	if (Type_Recipient == (CLASS_REQUEST | INTERFACE_RECIPIENT)) {
		if (RequestNo == SET_COMM_FEATURE) {
			...
			uint16_t bitmap = pInformation->USBwValues.w >> 8;
			if (bitmap & 0x1) // DTR
				onUsbConnected();
			else
				onUsbDisconnected();
