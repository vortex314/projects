...
Msgpack::Msgpack(int size) : Bytes(size) {
}

Msgpack::Msgpack(uint8_t* pb, int size) : Bytes(pb, size) {
}
Msgpack::~Msgpack() {
...
void Msgpack::pack(uint8_t* data, uint32_t size) {
    packRawHeader(size);
    uint32_t i;
    for (i = 0; i < size; i++) write(data[i]);
}

void Msgpack::packRawHeader(uint32_t data_size) {
    if (data_size < 32) {
        write((uint8_t) (TYPE_FIXRAW | (unsigned char) data_size));
    } else if (data_size < 0x10000) {
        write(TYPE_VALIABLE_RAW16);
        PACK_16BIT_VALUE(data_size);
    } else {
        write(TYPE_VALIABLE_RAW32);
