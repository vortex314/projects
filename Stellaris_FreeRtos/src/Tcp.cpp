...
Tcp::Tcp(Wiz5100* wiz,uint32_t socket) : _rxd(256), _txd(256) {
	led=&ledBlue;
    _wiz = wiz;
    _wiz->upStream(this);
...
int Tcp::recv(Bytes& msg) {
    if (_state != ST_CONNECTED) return 0;
    int count = 0;
...
Erc Tcp::write(Bytes* msg) {
    if (_state == ST_DISCONNECTED) return E_CONN_LOSS; // can accept data while connecting
    int count = 0;
    msg->offset(0);
...
Erc Tcp::flush() {
    ...
    uint16_t size;
    uint16_t realaddr;
    if (_state != ST_CONNECTED) return E_CONN_LOSS; // cannot send data while not connected
    if (_txd.hasData() == false) return E_LACK_RESOURCE;
    ...
    if (_txd.hasData()) queue( CMD_SEND); // restart event to send rest
