...
void Wiz5100::SpiInit(void) {

}

Erc Wiz5100::loadCommon(uint8_t mac[6],uint8_t ipAddress[4],uint8_t gtwAddr[4],uint8_t netmask[4]) {
    int i;
    int erc = 0;
    if (erc) return erc;
    for (i = 0; i < 4; i++)
...
Wiz5100::Wiz5100(uint32_t socket) : _timer(this) {
    _socket = socket;
...
int Wiz5100::exchange(uint32_t txd, uint32_t* rxd) {
 /*   while (TxBufFullSPI1());
    WriteSPI1(txd);
    while (DataRdySPI1() == false);
    *rxd = ReadSPI1();
    //    _delay_ms(1);*/
    return 0;
}
...
int Wiz5100::write(uint16_t address, uint8_t data) {
    uint32_t rxd;
    int erc = exchange((W5100_WRITE_OPCODE << 24) + (address << 8) + data, &rxd);
    if (erc) return erc;
    if (rxd != 0x00010203)
        return E_AGAIN;
    return 0;
...
int Wiz5100::read(uint16_t address, uint8_t* data) {
    uint32_t rxd;
    int erc;
    if ((erc = exchange((W5100_READ_OPCODE << 24) + (address << 8) + 0, &rxd)))
        return erc;
