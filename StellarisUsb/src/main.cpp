...
public:
	...
	void measure();
	int handler(Event* event);
	static void heapMemory(void* instance, Cmd cmd, Strpack& strp);
};

MeasureSeq measureSeq;
Flags memoryFlags = { T_STR, M_WRITE, QOS_1, I_OBJECT, true };
Prop memoryUsage("sys/heapUsage", &measureSeq, MeasureSeq::heapMemory,
		memoryFlags);

MeasureSeq::MeasureSeq() {
	PT_INIT(&t);
}
void MeasureSeq::measure() {
	struct mallinfo m;
	m = mallinfo();

}
int MeasureSeq::handler(Event* event) {
	PT_BEGIN ( &t )
		while (true) {
			timeout(5000);
			PT_YIELD_UNTIL(&t, timeout());
			Strpack strp(100);
			memoryUsage._flags.publishValue = true;
		}
	PT_END ( &t );
}
void MeasureSeq::heapMemory(void* instance, Cmd cmd, Strpack& strp) {
struct mallinfo m;
m = mallinfo();
